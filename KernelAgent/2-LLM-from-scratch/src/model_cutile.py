...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # =================================================================
        # [Smart Routing] Prefill vs Decoding
        # 프리필(T >= 64)일 때는 우리가 최적화한 cuTile FMHA(64x64 타일) 사용!
        # =================================================================
        if T >= 64:
            y = cutile_fmha(
                Q=q, K=k, V=v,
                tile_m=64, tile_n=64, # Phase 1에서 증명된 최고 성능 타일
                causal=True
            )
        else:
            # 디코딩 단계(T < 64)에서는 PyTorch FlashAttention 폴백
