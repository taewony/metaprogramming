# cuTile 기반 Windows-native LLM 추론 엔진 이식과 고정 컨텍스트 Agent Workload에서의 Prefix KV Cache 실증

**English title:** Windows-Native LLM Inference Engine Migration with cuTile and Prefix KV-Cache Evaluation for Fixed-Context Agent Workloads

**Author:** Taewon Kim

**Draft status:** KTCP-oriented revised draft. This version integrates measured prefix-cache results from `KernelAgent/3-micro-vllm/prefix_cache_results_cutile*.jsonl` and replaces the earlier assumption placeholders with measured values for Paper #1.

## 요약

본 연구는 Linux 중심 LLM 서빙 프레임워크의 Windows 환경 이식성 제약을 완화하고, LLM 추론 엔진 내부 동작을 교육 및 실험 목적으로 관찰할 수 있도록 CUDA Python 1.0 및 cuTile 기반 Windows-native 추론 엔진 **micro-vLLM**을 설계하고 구현하였다. micro-vLLM은 MatMul, FMHA, llm-from-scratch, nano-vLLM-style serving으로 이어지는 단계적 커널 마이그레이션 경로를 제공하며, Paged KV cache, prefix cache, Green Contexts, allocator thrashing 등 LLM serving loop의 주요 병목을 재현 가능한 형태로 드러낸다.

기존 실험에서 WSL2 FlashAttention backend는 평균 2138.55 tok/s를 기록하여 Windows-native cuTile backend 평균 463.35 tok/s보다 약 4.62배 높은 처리량을 보였다. 따라서 본 논문은 micro-vLLM을 production-grade Linux serving stack의 성능 대체물이 아니라, Windows-native 환경에서 LLM serving 구조를 학습하고 병목을 분석하기 위한 educational systems artifact로 위치시킨다. 추가로 본 연구는 Text-to-SQL agent, CUDA tutor, knowledge retrieval agent처럼 긴 system prompt, DB schema, `SKILL.md`, course context가 반복되는 **고정 컨텍스트 agent workload**를 정의하고, prefix KV cache의 효과를 실증하였다. RTX 5070 target PC에서 수행한 실험 결과, warm prefix cache는 computed prefill token 수를 모든 조건에서 64 token으로 감소시켰고, TTFT를 static prefix 1024/2048/3072 token 조건에서 각각 41.1%, 66.1%, 79.7% 감소시켰다. 반면 64-token generation benchmark에서는 decode loop가 전체 실행 시간을 지배하여 E2E throughput 개선은 일관되게 나타나지 않았다. 이는 prefix cache가 주로 prefill-dominated latency, 특히 TTFT를 개선하는 최적화임을 보여준다.

**핵심어:** LLM serving, CUDA Python 1.0, cuTile, nano-vLLM, prefix KV cache, Windows-native GPU computing, educational systems artifact

## Abstract

This study presents **micro-vLLM**, a Windows-native LLM inference engine artifact implemented with CUDA Python 1.0 and a cuTile-style kernel development workflow. Instead of claiming performance superiority over mature Linux serving stacks, micro-vLLM is designed as an educational systems artifact that exposes prefill, decode, paged KV cache, prefix caching, CUDA Green Contexts, and memory-allocation bottlenecks in a reproducible Windows GPU environment. The artifact follows a staged migration path from tiled MatMul to fused multi-head attention, a minimal LLM-from-scratch loop, and finally a nano-vLLM-style serving engine.

Prior measurements show that WSL2 FlashAttention achieves 2138.55 tok/s on average, approximately 4.62x faster than the current Windows-native cuTile backend at 463.35 tok/s. This result motivates a conservative framing: micro-vLLM is useful for migration, inspection, and education rather than immediate production replacement. We further evaluate fixed-context agent workloads, where requests share long static prefixes such as system prompts, database schemas, `SKILL.md` files, or course contexts. On an RTX 5070 target PC, warm prefix KV-cache reuse reduces computed prefill tokens to 64 in all tested settings and reduces TTFT by 41.1%, 66.1%, and 79.7% for 1024-, 2048-, and 3072-token static prefixes, respectively. The negative-control condition with changed prefixes yields 0.0% cache hit and near no-cache TTFT. End-to-end throughput does not consistently improve in the 64-token generation benchmark because decode dominates total runtime. These findings show that fixed-context educational and data-query agents benefit primarily through prefill and TTFT reduction, and that inference-engine education should analyze the full serving loop rather than isolated kernels alone.

**Keywords:** LLM serving, CUDA Python 1.0, cuTile, nano-vLLM, prefix KV cache, Windows-native GPU computing, educational systems artifact
...
본 연구는 성능 최고치를 목표로 하기보다, Windows-native 환경에서 LLM inference engine의 핵심 구조를 단계적으로 구현하고 관찰할 수 있는 **educational systems artifact**를 목표로 한다. micro-vLLM은 CUDA Python 1.0과 cuTile-style kernel workflow를 이용해 MatMul, FMHA, minimal autoregressive loop, paged KV cache, prefix cache, continuous batching, Green Contexts를 연결한다. 이 구조는 학습자가 GPU kernel, attention, KV cache, scheduler, decode loop, memory allocator가 전체 serving 성능에 어떻게 영향을 주는지 실험적으로 확인하도록 돕는다.

본 논문의 revised contribution은 **fixed-context agent workload**와 **prefix KV cache 실증**이다. Agent 시스템은 매 요청마다 system prompt, DB schema, tool contract, `SKILL.md`, policy, course material 등 긴 정적 prefix를 반복해서 사용한다. 이러한 prompt layout은 prefix KV cache가 효과를 발휘하기 좋은 구조이다. 본 연구는 RTX 5070 target PC에서 static prefix 길이를 1024, 2048, 3072 token으로 바꾸어 no-cache, warm-cache, prefix-changed 조건을 비교하고, prefix cache가 TTFT와 computed prefill token 수를 크게 줄임을 보인다.

## 2. 연구 질문과 기여

본 연구는 다음 질문을 다룬다.

**RQ1.** Windows-native CUDA Python/cuTile 기반 micro-vLLM은 LLM serving loop의 핵심 요소를 교육 및 실험 가능한 형태로 노출할 수 있는가?

**RQ2.** 고정 컨텍스트 agent workload에서 prefix KV cache는 computed prefill token 수와 TTFT를 얼마나 줄이는가?

**RQ3.** prefix cache, Green Contexts, shape padding 최적화는 전체 serving loop에서 어떤 성공과 실패 양상을 보이며, 이를 교육적 artifact로 어떻게 해석할 수 있는가?

본 논문의 기여는 다음과 같다.

1. **Windows-native educational systems artifact:** micro-vLLM을 MatMul, FMHA, llm-from-scratch, nano-vLLM-style serving으로 이어지는 단계적 이식 경로로 구성하였다.
2. **Fixed-context agent benchmark:** system prompt, DB schema, `SKILL.md`, course context가 반복되는 agent workload를 prefix cache 평가 대상으로 정의하였다.
3. **Prefix KV cache 실증:** 1024/2048/3072-token static prefix에서 warm cache가 computed prefill token을 64 token으로 줄이고 TTFT를 41.1%에서 79.7%까지 감소시킴을 측정하였다.
4. **Serving-loop bottleneck analysis:** WSL2 FlashAttention baseline, allocator thrashing, Green Contexts mixed result를 함께 제시하여 단일 kernel 최적화가 전체 serving 성능을 보장하지 않음을 보인다.

## 3. 배경

### 3.1 LLM Serving Loop
...
### 3.2 Paged KV Cache와 Prefix Cache
...
### 3.3 CUDA Python 1.0, cuTile, nano-vLLM
...
### 4.1 단계적 Kernel Migration

micro-vLLM의 개발 경로는 네 단계로 구성된다.

| Stage | Folder | 역할 |
| :--- | :--- | :--- |
| 0 | `KernelAgent/0-MatMul` | tiling, shared memory, swizzling, GEMM baseline 학습 |
| 1 | `KernelAgent/1-FMHA` | online softmax, causal masking, fused attention 구현 |
| 2 | `KernelAgent/2-LLM-from-scratch` | minimal autoregressive loop, KV cache, CUDA Graph 실험 |
...
### 4.2 micro-vLLM Serving Architecture

micro-vLLM은 Qwen-family model을 대상으로 lightweight serving loop를 구성한다. Scheduler는 waiting/running sequence를 관리하고, BlockManager는 KV cache block allocation과 hash 기반 prefix reuse를 담당한다. Sequence는 prompt token, generated token, block table, cached token count를 유지한다. ModelRunner는 prefill과 decode를 구분하여 input IDs, positions, cu_seqlens, slot mapping, block table을 구성한다.

Prefix cache 경로에서는 Scheduler가 sequence allocation 시 complete block hash를 검사한다. 동일한 prefix block이 존재하면 `seq.num_cached_tokens`가 증가하고, ModelRunner의 `prepare_prefill`은 cached token 이후의 suffix만 input으로 전달한다. 이때 `cu_seqlens_k`는 전체 context length를 유지하고 `cu_seqlens_q`는 새로 계산할 suffix length만 반영한다. 따라서 attention은 reused KV cache와 newly computed suffix를 함께 참조한다.

### 4.3 Fixed-Context Agent Workload
...
실험은 Windows 11 target PC에서 NVIDIA GeForce RTX 5070 GPU를 사용하여 수행하였다. GPU는 48 SM, 약 12GB VRAM, 48MB L2 cache를 갖는 환경으로 기록되었다. micro-vLLM은 Qwen-family model을 사용하며, 기존 paper draft와 analysis report의 baseline은 Qwen2.5/Qwen3 naming mismatch가 있으므로 최종 제출 전 정확한 local model directory와 Hugging Face config를 다시 고정해야 한다.

### 5.2 Prefix Cache Benchmark

Prefix cache 실험은 `KernelAgent/3-micro-vllm/bench_prefix_cache.py`로 수행하였다. 각 static prefix 길이마다 세 조건을 비교했다.

| Condition | 설명 |
| :--- | :--- |
| `no_cache` | 매 요청 전 persistent hash table을 비워 full prefill 수행 |
| `warm_cache` | prime request 이후 동일 static prefix를 재사용 |
| `prefix_changed` | prefix 첫 token을 변경하여 exact-prefix cache hit를 방지하는 negative control |
...
### 6.1 WSL2 FlashAttention Baseline과 Windows cuTile 비교
...
Negative control인 `prefix_changed` 조건은 모든 실험에서 0.0% cache hit를 보였고, TTFT가 no-cache 조건에 가깝게 회귀하였다. 이는 관찰된 개선이 measurement noise가 아니라 exact prefix reuse에 의해 발생했음을 확인한다.

반면 E2E throughput은 warm-cache 조건에서 일관되게 향상되지 않았다. 64-token generation benchmark에서는 prefill 이후 decode step이 63회 반복되며 전체 runtime을 지배한다. 특히 1024-token 조건에서는 warm-cache TTFT가 줄었지만 E2E latency가 증가하였다. 따라서 본 논문은 prefix KV cache의 효과를 **universal throughput speedup**이 아니라 **fixed-context workload의 prefill work 및 TTFT reduction**으로 해석한다.

### 6.3 Allocator Thrashing Negative Result
...
### 7.1 Fixed-Context Agent Workload의 의미
...
### 7.2 Educational Systems Artifact로서의 가치

micro-vLLM은 production vLLM보다 빠르지 않다. 그러나 KTCP 관점에서 본 연구의 실용적 가치는 다음에 있다.

- Windows-native 환경에서 LLM serving loop를 직접 실행하고 수정할 수 있다.
- MatMul부터 micro-vLLM까지 단계적 학습 경로가 있다.
- 성공한 최적화(prefix cache)와 실패한 최적화(shape padding)를 모두 재현할 수 있다.
- Green Contexts처럼 최신 CUDA 기능을 controlled workload에서 검토할 수 있다.
- 고정 컨텍스트 agent workload와 inference engine 내부 최적화의 연결을 보여준다.
...
1. 단일 RTX 5070 target PC 결과이므로 다른 GPU나 datacenter 환경으로 일반화하기 어렵다.
2. Windows-native cuTile backend는 WSL2 FlashAttention보다 크게 느리므로 production performance claim으로 해석하면 안 된다.
3. Prefix cache benchmark는 64-token generation으로 고정되어 decode-dominated 상황에서 E2E throughput 개선을 보이지 않았다.
4. Prefix cache 효과는 exact token-prefix stability에 의존한다.
5. Green Contexts 결과는 current sequential Python engine loop에서는 mixed result로 나타났다.
6. 최종 제출 전 model name, CUDA/PyTorch/cuTile version, random seed, benchmark command, raw log repository path를 엄밀히 고정해야 한다.
...
4. NVIDIA, "CUDA Python," NVIDIA CUDA Python Documentation.
5. NVIDIA, "Green Contexts," CUDA Programming Guide.
