...
```
• Implemented v11 deterministic SQL planner resolution, without --planner-llm.

  What changed:

  - Added activegraph/text-to-sql-agent/src/cli/sql_planner.py:15 with deterministic PlannerResolution.
  - Wired activegraph/text-to-sql-agent/src/cli/text_to_sql.py:137 so v11 runs:
    question.submitted -> resolve_sql_planner -> planner.resolved -> parse_intent.

  - Added v11 system models:
      - activegraph/text-to-sql-agent/agent/system-model.hospital.v11.yaml:649
      - activegraph/text-to-sql-agent/agent/system-model.techshop.v11.yaml:435

  - Updated packs to use v11 models.
  - Added graph-visible planner_resolution, decision_rationale, and clarification_request.
  - Ambiguous prompt path now returns clarification and does not generate SQL.
  - Updated plan.md, design-spec.md, system-model.v99.yaml, and sql_planner_design.md.

  Verified:

  - v11 focused tests: 3 passed
  - broader pack/context/v10/v11: 18 passed
  - full Text-to-SQL TDD: 58 passed
  - pack validate hospital-db: ok
  - pack validate techshop-db: ok
  - CLI smoke:
      - VIP 고객 총매출액 resolves with confirmed-order assumption and executes SQL.
      - 가장 많이 팔린 거 returns clarification with no SQL.
      - inspect shows planner graph objects clearly.
```

```
• Consolidated the two DB evals under activegraph/text-to-sql-agent/evals:

  - activegraph/text-to-sql-agent/evals/eval_manifest.yaml indexes both packs and their runnable/deferred eval sets.
  - activegraph/text-to-sql-agent/evals/hospital_consolidated_cases.jsonl: 29 hospital cases.
  - activegraph/text-to-sql-agent/evals/techshop_cases.jsonl: 23 techshop cases.
  - Kept existing hospital_cases.jsonl for compatibility.

  I also extended eval scoring in activegraph/text-to-sql-agent/src/cli/text_to_sql.py to check answer_source and
  planner-resolution expectations, and tightened v11 planner/rule behavior for the new evals.

  Verification passed:

  pytest consolidated evals: 2 passed, 58 deselected
  CLI hospital eval: 29 passed, 0 failed
  CLI techshop eval: 23 passed, 0 failed
  full text-to-SQL TDD: 60 passed
  ```
  ```
  • Fixed. Bare eval now uses the selected pack’s consolidated eval file instead of always defaulting to hospital cases.

  What changed:

  - activegraph/text-to-sql-agent/src/cli/text_to_sql.py now resolves default eval cases from eval_manifest.yaml.
  - run_eval JSON output now includes cases_file.
  - Added regression in tests/test_activegraph_text_to_sql_tdd.py.

  Verified your exact flow:

  text-to-sql> pack use techshop-db
  default pack: techshop-db
  text-to-sql> eval
  summary: 23 passed, 0 failed

  Also verified hospital default selection:

  --pack hospital-db text-to-sql eval --json
  passed: 29
  failed: 0
  cases_file: hospital_consolidated_cases.jsonl

  Tests:

  - Focused regression set: 4 passed, 57 deselected
  - Full suite attempt was blocked by pytest temp-directory permission errors in this Windows environment, not by
    assertion failures in this change path.
```